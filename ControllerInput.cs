...
public class ControllerInput
{
    ...
    //private string deviceID = "000666766D07"; // dummy device
    private string deviceID = "000666D1FCBE"; // real device
    private string virtualCOMPort = "";

    // autodetect COM Port for a given deviceID
    public void AutoDetect()
    {
        // requires System.Management
        List<ManagementObject> lsMObj = new List<ManagementObject>(); 

        // query for list of working devices on Windows Device Manager
        string sQuery = "SELECT * FROM Win32_PnPEntity WHERE ConfigManagerErrorCode = 0";

        // create search based on query
        //ManagementObjectSearcher moSearcher = new ManagementObjectSearcher(new ObjectQuery(sQuery));
        try
        {
            // create list from search based on query
            lsMObj = new ManagementObjectSearcher(sQuery).Get().Cast<ManagementObject>().ToList();

            // iterate through the list to try and match deviceID
            foreach (ManagementObject obj in lsMObj)
            {
                try
                {
                    // caption name of device
                    string caption = obj["Caption"].ToString();

                    // device ID and COM port caption
                    if (obj.ToString().Contains(deviceID) && caption.Contains("(COM"))
                    {
                        // pattern to match
                        ...
                        // match expression pattern against string
                        Match m = r.Match(caption);

                        if (m.Success)
                        {
                            virtualCOMPort = m.ToString();
                            //Trace.WriteLine(virtualCOMPort + " found at position " + m.Index);  // diagnostics
                            //m = m.NextMatch();
                        }
                ...
                catch (Exception ex1)
                {
                    ...
                    Trace.WriteLine("Some exception..." + ex1);
                }
            }
        ...
        {
            Trace.WriteLine("Some exception..." + ex2);
        }

        // diagnostics
        //if (virtualCOMPort == "")
        //    Trace.WriteLine("No device found");
        //else
        //    Trace.WriteLine("Device " + deviceID + " found: " + virtualCOMPort);
    }
    ...
    public void OpenCommunication()
    {
        ...
        // try autodetect to figure virtual COM port
        try
        {
            this.AutoDetect();
        }
        ...
        {
            // use usual port if autodetect fails
            virtualCOMPort = "COM5";
            Trace.WriteLine("Autodetect failed, trying COM5...");
            Trace.WriteLine("Exception: " + ex);
        }

        _serialPort = new SerialPort(virtualCOMPort, 115200, Parity.None, 8, StopBits.One);
